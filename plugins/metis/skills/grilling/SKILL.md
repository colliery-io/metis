...
- **Decisions are not numbered either.** The shorthand `D1`/`D2`/`D3` for "the third decision" is the same failure wearing a suit: two rounds later "let's revisit D3" resolves to nothing and nothing ever closed it. A decision that has been recorded is its ADR short code (`PROJ-A-0003`). A decision area still open is named by what it decides ("Session storage backend"). A choice not worth an ADR is prose in the document's design section, with no identifier at all.
