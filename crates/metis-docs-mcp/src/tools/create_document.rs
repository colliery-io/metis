...
impl CreateDocumentTool {
    pub async fn call_tool(&self) -> std::result::Result<CallToolResult, CallToolError> {
        ...
        // Parse document type
        let doc_type = DocumentType::from_str(&self.document_type)
            .map_err(|_| CallToolError::new(std::io::Error::new(
        ...
        let result = match doc_type {
            ...
            DocumentType::Initiative => {
                let parent_id = self.parent_title.as_ref()
            ...
            DocumentType::Task => {
                ...
                creation_service.create_task(config, parent_id, parent_id).await
                    .map_err(|e| CallToolError::new(e))?
            }
            DocumentType::Adr => {
