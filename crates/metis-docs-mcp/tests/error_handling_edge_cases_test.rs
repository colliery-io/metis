...
async fn test_large_title_edge_cases() -> Result<()> {
    ...
    if result.is_ok() {
        ...
        if let Some(strategy) = strategies.iter().find(|s| s.title == long_title) {
            println!("  - Full title preserved in database: {} chars", strategy.title.len());
...
async fn test_special_characters_edge_cases() -> Result<()> {
    ...
    let test_titles = vec![
        ...
        ("Title with \"quotes\" and 'apostrophes'", "Quote character test"),
        ("Title with <HTML> tags & entities", "HTML special chars test"),
        ("Title with\nnewlines\tand\ttabs", "Whitespace character test"),
        ("Tïtlé wîth äccénts ànd ñ spëcial", "Accented character test"),
        ("Title.with.dots...and___underscores---", "Filename special chars test"),
    ...
    for (title, description) in test_titles {
        ...
        if result.is_ok() {
            ...
            let db = helper.get_database()?;
            let mut repo = db.repository().map_err(|e| anyhow::anyhow!("Repository error: {}", e))?;
            let adrs = repo.find_by_type("adr").map_err(|e| anyhow::anyhow!("Find error: {}", e))?;
...
async fn test_numeric_boundary_values() -> Result<()> {
    ...
    for (complexity, should_succeed, description) in complexity_values {
        ...
        if should_succeed {
            if result.is_err() {
                println!("❌ {} failed with error: {:?}", description, result.err().unwrap());
...
async fn test_concurrent_modifications() -> Result<()> {
    ...
    let (trans_result1, trans_result2) = tokio::join!(
        transition1.call_tool(),
        transition2.call_tool()
    );
    ...
    if let Some(strategy) = strategies.iter().find(|s| s.title == "Concurrent Test Strategy") {
        println!("Final phase after concurrent transitions: {}", strategy.phase);
    }
