...
async fn test_tui_archive_initiative_cascade() -> Result<()> {
    ...
    assert!(result.is_ok(), "Strategy creation should succeed: {:?}", result);
    ...
    assert!(result.is_ok(), "Initiative creation should succeed: {:?}", result);
    ...
    let result = app.create_child_document().await;
    assert!(result.is_ok(), "First task creation should succeed: {:?}", result);
    ...
    let result = app.create_child_document().await;
    assert!(result.is_ok(), "Second task creation should succeed: {:?}", result);
    ...
    assert_eq!(tasks.iter().filter(|t| t.archived).count(), 0, "No tasks should be archived");
    ...
    let strategy_dir = helper.metis_dir.join("strategies").join("test-strategy-for-archive");
    let initiative_dir = strategy_dir.join("initiatives").join("test-initiative-for-archive");
    ...
    if result.is_ok() {
        ...
        assert!(!strategies_after[0].archived, "Strategy should still be active");
        
        // Initiative should be archived
        assert!(initiatives_after[0].archived, "Initiative should be archived");
        
        // Both tasks should be archived due to cascade
        let archived_tasks = tasks_after.iter().filter(|t| t.archived).count();
        assert_eq!(archived_tasks, 2, "Both tasks should be archived due to cascade");
        ...
        assert!(strategies_dir.exists(), "Strategies directory should still exist");
        ...
        let archived_strategy_dir = archived_dir.join("strategies").join("test-strategy-for-archive");
        let archived_initiative_dir = archived_strategy_dir.join("initiatives").join("test-initiative-for-archive");
        ...
        if err_msg.contains("Directory not empty") || err_msg.contains("os error 66") {
            ...
            return Err(anyhow::anyhow!("Archive failed with directory not empty error - this confirms the bug"));
        }
...
async fn test_tui_archive_empty_initiative() -> Result<()> {
    ...
    let result = app.archive_selected_document().await;
    assert!(result.is_ok(), "Empty initiative archive should succeed: {:?}", result);
...
async fn test_tui_archive_strategy_with_partial_archive() -> Result<()> {
    ...
    let result = app.archive_selected_document().await;
    assert!(result.is_ok(), "Individual task archive should succeed: {:?}", result);
    ...
    assert!(!db_strategies_after_single[0].archived, "Strategy should still be active");
    assert!(!db_initiatives_after_single[0].archived, "Initiative should still be active");
    ...
    if result.is_ok() {
        ...
        assert!(db_strategies_final[0].archived, "Strategy should be archived");
        assert!(db_initiatives_final[0].archived, "Initiative should be archived due to cascade");
        
        let final_archived_tasks = db_tasks_final.iter().filter(|t| t.archived).count();
        assert_eq!(final_archived_tasks, 2, "Both tasks should be archived (1 individual + 1 cascaded)");
        
        println!("✅ Full cascade archiving successful");
        println!("   - Strategy: archived");
        println!("   - Initiative: archived (cascaded)");
        println!("   - Tasks: {} archived (1 individual + 1 cascaded)", final_archived_tasks);
        
    } else {
        println!("❌ Strategy archive failed: {:?}", result);
        return Err(anyhow::anyhow!("Strategy archive should have succeeded: {:?}", result));
    }
