...
async fn test_edit_and_save_strategy() -> Result<()> {
    ...
    assert!(content.contains("This is my edited content"), "Should contain edited content");
    assert!(content.contains("Increase performance"), "Should contain first goal");
    assert!(content.contains("Improve user experience"), "Should contain second goal");
    
    // Verify frontmatter is still intact
    assert!(content.contains("level: strategy"), "Should still have strategy level");
    assert!(content.contains("title: \"My Strategy\""), "Should still have title");
    
    // Verify the old template content is NOT present
    assert!(!content.contains("Problem Statement"), "Should not contain template content");
    assert!(!content.contains("Success Metrics"), "Should not contain template content");
