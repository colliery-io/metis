...
use metis_core::{Document, domain::configuration::FlightLevelConfig};
...
async fn test_ctrl_s_saves_strategy_document() -> Result<()> {
    ...
    assert!(!strategy_dirs.is_empty(), "Should have at least one strategy directory");
...
async fn test_ctrl_s_save_in_streamlined_configuration() -> Result<()> {
    ...
    let initiatives_dir = helper.metis_dir().join("strategies").join("NULL").join("initiatives");
    assert!(initiatives_dir.exists(), "NULL strategy initiatives directory should exist");
    ...
    assert!(!initiative_dirs.is_empty(), "Should have at least one initiative directory");
...
async fn test_ctrl_s_save_in_direct_configuration() -> Result<()> {
    ...
    let tasks_dir = helper.metis_dir().join("strategies").join("NULL").join("initiatives").join("NULL").join("tasks");
