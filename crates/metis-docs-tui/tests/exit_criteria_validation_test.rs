...
async fn test_tui_phase_transition_exit_criteria() -> Result<()> {
    ...
    let (col, _) = app.selection_state.get_current_selection(BoardType::Strategy);
    ...
    if let Some(error) = app.error_message() {
        ...
        if board.columns[1].items.iter().any(|item| item.title() == title) {
            println!("⚠️  Phase transition succeeded despite unmet exit criteria");
        } else if board.columns[0].items.iter().any(|item| item.title() == title) {
...
async fn test_tui_phase_transition_keyboard_shortcut() -> Result<()> {
    ...
    assert!(task_board.columns[0].items.iter().any(|item| item.title() == task_title));
    ...
    if task_board.columns[1].items.iter().any(|item| item.title() == task_title) {
...
async fn test_tui_exit_criteria_persistence() -> Result<()> {
    ...
    let strategy_exists = board.columns.iter()
