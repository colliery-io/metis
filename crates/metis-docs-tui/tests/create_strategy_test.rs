...
async fn test_create_strategy() -> Result<()> {
    ...
    assert_eq!(board.columns[0].items.len(), 0, "Strategy board should be empty initially");
    ...
    let board = &app.ui_state.strategy_board;
    assert_eq!(board.columns[0].items.len(), 1, "Should have 1 strategy in Shaping column");
    ...
    assert!(content.contains("level: strategy") || content.contains("document_type: strategy"), 
            "Should have strategy document type");
    assert!(content.contains("title: \"Test Strategy\""), "Should have correct title");
    assert!(content.contains("#phase/shaping"), "Should be in shaping phase");
