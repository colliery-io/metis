...
async fn test_basic_board_navigation() -> Result<()> {
    ...
    // Initialize services
    app.document_service = Some(metis_docs_tui::services::DocumentService::new(metis_dir.clone()));
    app.sync_service = Some(metis_docs_tui::services::SyncService::new(metis_dir.clone()));
...
async fn test_selection_movement() -> Result<()> {
    ...
    let (col, row) = app.selection_state.get_current_selection(BoardType::Strategy);
    assert_eq!(col, 0);
    assert_eq!(row, 0);
