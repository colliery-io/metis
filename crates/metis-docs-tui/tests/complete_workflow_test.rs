...
async fn test_complete_flight_levels_workflow() -> Result<()> {
    ...
    assert!(vision_path.exists(), "Vision document should exist after project initialization");
    
    let vision_content = std::fs::read_to_string(&vision_path)?;
    assert!(vision_content.contains("level: vision"), "Should be a vision document");
    ...
    for ch in strategy_title.chars() {
        app.ui_state.input_title.handle_event(&crossterm::event::Event::Key(
    ...
    let strategy_dirs: Vec<_> = std::fs::read_dir(&strategies_dir)?
        ...
        .collect();
    assert_eq!(strategy_dirs.len(), 1, "Should have exactly one strategy directory");
    ...
    assert!(strategy_content.contains("level: strategy"), "Should be a strategy document");
    assert!(strategy_content.contains("title: \"Improve Customer Experience\""), "Should have correct title");
    assert!(strategy_content.contains("#phase/shaping"), "Should start in shaping phase");
    
    // Verify in database
    let db = metis_core::dal::Database::new(
        helper.metis_dir.join("metis.db").to_str().unwrap()
    ).map_err(|e| anyhow::anyhow!("Database error: {}", e))?;
    
    let mut repo = db.repository()
        .map_err(|e| anyhow::anyhow!("Repository error: {}", e))?;
    
    let db_strategies = repo.find_by_type("strategy")
    ...
    assert_eq!(strategy_board.columns[0].items.len(), 1, "Should have 1 strategy in Shaping column");
    assert_eq!(strategy_board.columns[0].items[0].prelude, "Improve Customer Experience");
    
    // Select the strategy and create a child initiative
    // The selection should already be on the strategy (0,0)
    let (col_idx, item_idx) = app.selection_state.get_current_selection(BoardType::Strategy);
    ...
    let strategy_initiative_dir = helper.metis_dir
        .join("strategies")
        .join("improve-customer-experience")
        .join("initiatives");
    assert!(strategy_initiative_dir.exists(), "Strategy initiatives directory should exist");
    
    let initiative_dirs: Vec<_> = std::fs::read_dir(&strategy_initiative_dir)?
        .filter_map(|entry| entry.ok())
        .filter(|entry| entry.path().is_dir())
        .collect();
    assert_eq!(initiative_dirs.len(), 1, "Should have exactly one initiative directory");
    ...
    assert!(initiative_content.contains("level: initiative"), "Should be an initiative document");
    assert!(initiative_content.contains("title: \"Redesign User Onboarding\""), "Should have correct title");
    assert!(initiative_content.contains("#phase/discovery"), "Should start in discovery phase");
    assert!(initiative_content.contains("parent: improve-customer-experience"), "Should reference parent strategy");
    
    // Verify in database
    let db_initiatives = repo.find_by_type("initiative")
        .map_err(|e| anyhow::anyhow!("Find error: {}", e))?;
    assert_eq!(db_initiatives.len(), 1, "Should have 1 initiative in database");
    ...
    assert_eq!(initiative_board.columns[0].items.len(), 1, "Should have 1 initiative in Discovery column");
    assert_eq!(initiative_board.columns[0].items[0].prelude, "Redesign User Onboarding");
    
    // The selection should already be on the initiative (0,0)
    let (col_idx, item_idx) = app.selection_state.get_current_selection(BoardType::Initiative);
    assert_eq!(col_idx, 0, "Should be selecting first column");
    assert_eq!(item_idx, 0, "Should be selecting first item");
    ...
    assert!(task_content.contains("level: task"), "Should be a task document");
    assert!(task_content.contains("title: \"Create wireframes for onboarding flow\""), "Should have correct title");
    assert!(task_content.contains("#phase/todo"), "Should start in todo phase");
    assert!(task_content.contains("parent: redesign-user-onboarding"), "Should reference parent initiative");
    
    // Verify in database
    let db_tasks = repo.find_by_type("task")
        .map_err(|e| anyhow::anyhow!("Find error: {}", e))?;
    ...
    assert_eq!(task_board.columns[0].items.len(), 2, "Should have 2 tasks in Todo column");
    ...
    let db_tasks_after_single_archive = repo.find_by_type("task")
        .map_err(|e| anyhow::anyhow!("Find error: {}", e))?;
    let active_tasks: Vec<_> = db_tasks_after_single_archive.iter()
        .filter(|task| !task.archived)
        .collect();
    let archived_tasks: Vec<_> = db_tasks_after_single_archive.iter()
        .filter(|task| task.archived)
        .collect();
    
    assert_eq!(active_tasks.len(), 1, "Should have 1 active task after single archive");
    assert_eq!(archived_tasks.len(), 1, "Should have 1 archived task after single archive");
    
    // Verify other documents are still active
    let db_initiatives_after_single_archive = repo.find_by_type("initiative")
        .map_err(|e| anyhow::anyhow!("Find error: {}", e))?;
    let db_strategies_after_single_archive = repo.find_by_type("strategy")
        .map_err(|e| anyhow::anyhow!("Find error: {}", e))?;
    
    assert_eq!(db_initiatives_after_single_archive[0].archived, false, "Initiative should still be active");
    assert_eq!(db_strategies_after_single_archive[0].archived, false, "Strategy should still be active");
    
    println!("✅ Single task archived successfully");
    println!("   - Active tasks: {}", active_tasks.len());
    println!("   - Archived tasks: {}", archived_tasks.len());
    println!("   - Initiative still active: {}", !db_initiatives_after_single_archive[0].archived);
    println!("   - Strategy still active: {}", !db_strategies_after_single_archive[0].archived);
    ...
    println!("  - Initiative archived: {}", db_initiatives_final[0].archived);
    
    let final_archived_tasks: Vec<_> = db_tasks_final.iter()
        .filter(|task| task.archived)
        .collect();
    ...
    if db_strategies_final[0].archived {
        ...
        assert_eq!(final_archived_tasks.len(), 2, "Both tasks should be archived due to cascade");
        ...
        assert_eq!(final_archived_tasks.len(), 1, "Should still have the 1 task we archived manually");
        assert_eq!(db_initiatives_final[0].archived, false, "Initiative should still be active due to failed cascade");
    ...
    assert_eq!(final_strategy_board.columns[0].items.len(), 0, "Strategy board should be empty after archive");
    
    app.jump_to_initiative_board();
    app.load_documents().await?;
    let final_initiative_board = &app.ui_state.initiative_board;
    assert_eq!(final_initiative_board.columns[0].items.len(), 0, "Initiative board should be empty after archive");
    
    app.jump_to_task_board();
    app.load_documents().await?;
    let final_task_board = &app.ui_state.task_board;
    assert_eq!(final_task_board.columns[0].items.len(), 0, "Task board should be empty after archive");
