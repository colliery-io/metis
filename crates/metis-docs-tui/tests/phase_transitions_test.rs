...
async fn validate_phase_transition(
    ...
    assert_eq!(
        db_doc.filepath, full_path.to_string_lossy(),
        "Database filepath should be '{}' but was '{}'",
        full_path.display(), db_doc.filepath
    );
...
async fn test_strategy_phase_transitions() -> Result<()> {
    ...
    let strategy_dir = helper.metis_dir.join("strategies").join("digital-transformation-strategy");
    ...
    assert!(file_content.contains("parent:"), "File should contain parent field");
    ...
    for (i, (phase, expected_col)) in phases.iter().zip(expected_columns.iter()).enumerate() {
        ...
        while app.selection_state.get_current_selection(BoardType::Strategy).0 != current_col {
        ...
        validate_phase_transition(&helper, "strategies/digital-transformation-strategy/strategy.md", phase, "strategy", &strategy_id).await?;
    }
...
async fn test_initiative_phase_transitions() -> Result<()> {
    ...
    let initiative_dir = helper.metis_dir
        .join("strategies")
    ...
    for (i, (phase, expected_col)) in phases.iter().zip(expected_columns.iter()).enumerate() {
        ...
        let current_col = if i == 0 { 0 } else { expected_columns[i-1] };
        while app.selection_state.get_current_selection(BoardType::Initiative).0 != current_col {
            app.move_selection_right();
        }
        ...
        // Validate the phase transition
        validate_phase_transition(&helper, "strategies/growth-strategy/initiatives/launch-new-product-line/initiative.md", phase, "initiative", &initiative_id).await?;
    }
...
async fn test_adr_phase_transitions() -> Result<()> {
    ...
    let db_adrs = repo.find_by_type("adr")
        .map_err(|e| anyhow::anyhow!("Find error: {}", e))?;
...
async fn test_invalid_phase_transitions() -> Result<()> {
    ...
    if let Some(error) = app.error_message() {
        ...
        assert!(error.contains("Invalid phase transition") || error.contains("cannot transition"), 
        ...
        assert_eq!(db_adrs[0].phase, "decided", "ADR should still be in decided phase");
