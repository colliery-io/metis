...
impl SyncCommand {
    pub async fn execute(&self) -> Result<()> {
        ...
        for result in &sync_results {
            match result {
                metis_core::application::services::synchronization::SyncResult::Imported { filepath } => {
                    println!("✓ Imported: {}", filepath);
                    imported += 1;
                }
                metis_core::application::services::synchronization::SyncResult::Updated { filepath } => {
                    println!("✓ Updated: {}", filepath);
                    updated += 1;
                }
                metis_core::application::services::synchronization::SyncResult::Deleted { filepath } => {
                    println!("✓ Deleted: {}", filepath);
                    deleted += 1;
                }
                metis_core::application::services::synchronization::SyncResult::UpToDate { filepath } => {
                    println!("• Up to date: {}", filepath);
                    up_to_date += 1;
                }
                metis_core::application::services::synchronization::SyncResult::NotFound { filepath } => {
                    println!("? Not found: {}", filepath);
                }
                metis_core::application::services::synchronization::SyncResult::Error { filepath, error } => {
