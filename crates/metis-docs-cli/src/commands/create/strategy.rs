...
async fn get_vision_document_id(workspace_dir: &Path, vision_slug: &str) -> Result<DocumentId> {
    ...
    if !vision_path.exists() {
        anyhow::bail!("Vision document not found: {}. Expected vision.md in workspace root.", vision_path.display());
    }
    
    // Parse the vision document to get its actual ID
    let vision = Vision::from_file(&vision_path).await
...
mod tests {
    ...
    async fn test_create_new_strategy_with_workspace() {
        ...
        assert_eq!(strategy_dirs.len(), 1, "Expected exactly one strategy directory");
        ...
        assert!(strategy_file.exists(), "strategy.md not found in strategy directory");
        ...
        assert!(parsed_strategy.is_ok(), "Failed to parse strategy file: {:?}", parsed_strategy.err());
