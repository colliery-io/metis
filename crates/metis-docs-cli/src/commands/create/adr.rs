...
mod tests {
    ...
    async fn test_create_new_adr_with_workspace() {
        ...
        assert!(filename.starts_with("001-"), "ADR filename should start with 001-");
        assert!(filename.contains("test-adr"), "ADR filename should contain the slug");
        ...
        assert!(parsed_adr.is_ok(), "Failed to parse ADR file: {:?}", parsed_adr.err());
