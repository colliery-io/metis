use metis_core::application::services::document::creation::{DocumentCreationConfig, DocumentCreationService};
...
async fn test_document_id_matches_path() {
    ...
    WorkspaceInitializationService::initialize_workspace(&workspace_dir, "test-project").await.unwrap();
    ...
    assert_eq!(strategy_result.document_id.to_string(), "semantic-search-rag-strategy");
    
    // Verify the directory path matches the ID
    let expected_path = workspace_dir.join("strategies").join("semantic-search-rag-strategy");
    assert!(expected_path.exists());
    assert!(expected_path.join("strategy.md").exists());
    
    // Verify discovery service finds it with the correct ID
    let found = discovery_service.find_document_by_id("semantic-search-rag-strategy").await.unwrap();
...
async fn test_initiative_id_path_consistency() {
    ...
    let strategy = creation_service.create_strategy(strategy_config).await.unwrap();
    ...
    assert_eq!(initiative_result.document_id.to_string(), "build-ai-powered-search-retrieval");
...
async fn test_task_id_path_consistency() {
    ...
    let task_result = creation_service
        .create_task(task_config, &strategy.document_id.to_string(), &initiative.document_id.to_string())
        .await
        .unwrap();
    
    // Verify ID generation handles special chars (truncated at word boundary)
    assert_eq!(task_result.document_id.to_string(), "setup-ci-cd-pipeline-testing");
...
async fn test_adr_id_consistency() {
    ...
    let adr_config = DocumentCreationConfig {
        ...
        parent_id: None,
        tags: vec![Tag::Label("architecture".to_string()), Tag::Label("database".to_string())],
        phase: None,
        complexity: None,
        risk_level: None,
    };
...
async fn test_long_title_id_path_consistency() {
    ...
    let result = creation_service.create_strategy(strategy_config).await.unwrap();
    ...
    let strategy_dir = workspace_dir.join("strategies").join(&result.document_id.to_string());
    assert!(strategy_dir.exists());
    assert!(strategy_dir.join("strategy.md").exists());
    
    // Verify we can find it by ID
    let discovery_service = DocumentDiscoveryService::new(&workspace_dir);
    let found = discovery_service.find_document_by_id(&result.document_id.to_string()).await;
    assert!(found.is_ok());
}
...
async fn test_unicode_title_id_path_consistency() {
    ...
    for (i, title) in unicode_titles.iter().enumerate() {
        ...
        assert!(id.chars().all(|c| c.is_alphanumeric() || c == '-'), "ID should only contain alphanumeric and hyphens: {}", id);
        assert!(!id.contains("--"), "ID should not contain double hyphens: {}", id);
        
        // Verify path exists
        let strategy_dir = workspace_dir.join("strategies").join(&result.document_id.to_string());
        assert!(strategy_dir.exists(), "Directory should exist for Unicode title: {}", title);
        assert!(strategy_dir.join("strategy.md").exists());
    }
}
...
async fn test_regression_id_path_mismatch_bug() {
    ...
    let strategy_config = DocumentCreationConfig {
        ...
        parent_id: None,
        tags: vec![Tag::Label("search".to_string()), Tag::Label("ai".to_string())],
        phase: None,
        complexity: None,
    ...
    assert!(strategy_dir.exists(), "Strategy directory should exist at path matching ID");
    assert!(strategy_dir.join("strategy.md").exists(), "Strategy file should exist");
    
    // Verify the frontmatter ID matches
    let strategy_content = std::fs::read_to_string(strategy_dir.join("strategy.md")).unwrap();
    assert!(strategy_content.contains(&format!("id: {}", strategy_id)), "Frontmatter ID should match");
    
    // Verify discovery by ID works
    let found = discovery_service.find_document_by_id(&strategy_id).await.unwrap();
    ...
    let initiative_config = DocumentCreationConfig {
        ...
        parent_id: Some(metis_core::domain::documents::types::DocumentId::from(strategy_id.clone())),
        tags: vec![],
        phase: None,
        complexity: Some(Complexity::L),
        risk_level: None,
    ...
    assert!(initiative_path.exists(), "Initiative should be created under correct parent strategy path");
    assert!(initiative_path.join("initiative.md").exists(), "Initiative file should exist");
}