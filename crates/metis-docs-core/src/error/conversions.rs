...
impl UserFriendlyError for MetisError {
    fn to_user_message(&self) -> String {
        match self {
            MetisError::DocumentNotFound { id } => {
                format!("Document '{}' could not be found. It may have been moved or deleted.", id)
            }
            ...
            MetisError::MissingRequiredField { field } => {
                format!("Required field '{}' is missing. Please provide this information.", field)
            }
            MetisError::TemplateNotFound { template } => {
                format!("Template '{}' could not be found. Please check your template configuration.", template)
            }
            MetisError::ValidationFailed { message } => {
                format!("Validation failed: {}", message)
            }
            MetisError::ExitCriteriaNotMet { missing_count, total_count } => {
            ...
            MetisError::Connection(e) => {
                format!("Database connection error: {}. Please check your database configuration.", e)
            }
            MetisError::Io(e) => {
                format!("File system error: {}. Please check file permissions and try again.", e)
            }
            MetisError::Json(e) => {
                format!("JSON parsing error: {}. Please check the document format.", e)
            }
            MetisError::Yaml(e) => {
                format!("YAML parsing error: {}. Please check the document format.", e)
            }
            _ => {
    ...
    fn error_category(&self) -> ErrorCategory {
        match self {
            ...
            MetisError::Database(_) 
            | MetisError::Connection(_) => ErrorCategory::Database,
            ...
            MetisError::Json(_) 
            | MetisError::Yaml(_) => ErrorCategory::Document,
...
mod tests {
    ...
    fn test_error_context() {
        ...
        assert!(with_context.unwrap_err().to_string().contains("Reading configuration file"));
    }

    #[test]
    fn test_user_friendly_error_document_not_found() {
        let error = MetisError::DocumentNotFound { id: "test-doc".to_string() };
    ...
    fn test_user_friendly_error_validation() {
        let error = MetisError::ValidationFailed { message: "test validation".to_string() };
        let message = error.to_user_message();
