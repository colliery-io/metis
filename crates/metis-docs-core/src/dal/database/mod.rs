...
impl Database {
    ...
    pub fn get_connection(&self) -> Result<SqliteConnection, Box<dyn std::error::Error + Send + Sync>> {
    ...
    pub fn repository(&self) -> Result<repository::DocumentRepository, Box<dyn std::error::Error + Send + Sync>> {
