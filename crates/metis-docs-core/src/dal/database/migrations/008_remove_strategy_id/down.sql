-- Add strategy_id column back to documents table
-- SQLite doesn't support ADD COLUMN with all constraints, so recreate
...
-- Copy data back (strategy_id will be NULL for all rows)
INSERT INTO documents_temp (
    ...
    content, phase, strategy_id, initiative_id, short_code
)
SELECT
    filepath, id, title, document_type, created_at, updated_at,
    archived, exit_criteria_met, file_hash, frontmatter_json,
    content, phase, NULL, initiative_id, short_code
FROM documents;
...
-- Recreate indexes (with strategy_id)
