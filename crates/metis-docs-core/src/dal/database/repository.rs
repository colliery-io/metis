...
impl DocumentRepository {
    ...
    pub fn find_by_type_and_phase(&mut self, doc_type: &str, phase_name: &str) -> Result<Vec<Document>> {
        use schema::documents::dsl::*;
...
mod tests {
    ...
    fn test_create_and_find_document() {
        ...
        let created = repo.create_document(new_doc).expect("Failed to create document");
        ...
        let found = repo.find_by_filepath("/test/doc.md")
            .expect("Failed to find document")
            .expect("Document not found");
        assert_eq!(found.id, "test-doc-1");
        
        // Test find by id
        let found_by_id = repo.find_by_id("test-doc-1")
            .expect("Failed to find document")
            .expect("Document not found");
    ...
    fn test_update_document() {
        ...
        let new_doc = create_test_document();
        let mut created = repo.create_document(new_doc).expect("Failed to create document");
        
        // Update the document
        created.title = "Updated Title".to_string();
        created.updated_at = 1609462800.0; // 1 hour later
        
        let updated = repo.update_document("/test/doc.md", &created)
    ...
    fn test_delete_document() {
        ...
        let new_doc = create_test_document();
        repo.create_document(new_doc).expect("Failed to create document");
        
        // Delete the document
        let deleted = repo.delete_document("/test/doc.md")
        ...
        let deleted_again = repo.delete_document("/test/doc.md")
            .expect("Failed to delete document");
    ...
    fn test_document_relationships() {
        ...
        repo.create_document(parent_doc).expect("Failed to create parent");
        ...
        repo.create_document(child_doc).expect("Failed to create child");
        ...
        repo.create_relationship(relationship).expect("Failed to create relationship");
        
        // Test find children
        let children = repo.find_children("parent-1").expect("Failed to find children");
        assert_eq!(children.len(), 1);
        assert_eq!(children[0].id, "child-1");
        
        // Test find parent
        let parent = repo.find_parent("child-1")
    ...
    fn test_find_by_type() {
        ...
        repo.create_document(vision_doc).expect("Failed to create vision");
        repo.create_document(strategy_doc).expect("Failed to create strategy");
        ...
        let strategies = repo.find_by_type("strategy").expect("Failed to find strategies");
    ...
    fn test_document_not_found() {
        ...
        let found = repo.find_by_filepath("/nonexistent.md")
            .expect("Failed to search for document");
        assert!(found.is_none());
        
        let found_by_id = repo.find_by_id("nonexistent")
            .expect("Failed to search for document");
