...
use crate::domain::documents::{
    ...
    traits::Document as DocumentTrait,
};
...
impl<'a> SyncService<'a> {
    ...
    pub async fn import_from_file<P: AsRef<Path>>(&mut self, file_path: P) -> Result<Document> {
        ...
        let document_obj = DocumentFactory::from_file(&file_path).await
        ...
        let new_doc = self.domain_to_database_model(document_obj.as_ref(), &path_str, file_hash, updated_at, content)?;
    ...
    pub async fn export_to_file(&mut self, filepath: &str) -> Result<()> {
        ...
        let db_doc = self.db_service
            .find_by_filepath(filepath)?
    ...
    fn domain_to_database_model(
        ...
        Ok(NewDocument {
            ...
            file_hash,
            frontmatter_json: serde_json::to_string(&core.metadata)
                .map_err(MetisError::Json)?,
...
mod tests {
    ...
    async fn test_import_from_file() {
        ...
        let doc = sync_service.import_from_file(&file_path).await.expect("Failed to import");
        assert_eq!(doc.title, "Test Document");
        assert_eq!(doc.document_type, "vision");
        
        // Verify it's in the database
        assert!(db_service.document_exists(&file_path.to_string_lossy()).expect("Failed to check"));
    }
    ...
    async fn test_sync_file_operations() {
        ...
        assert_eq!(result, SyncResult::NotFound { filepath: path_str.clone() });
        ...
        let result = sync_service.sync_file(&file_path).await.expect("Failed to sync");
        assert_eq!(result, SyncResult::Imported { filepath: path_str.clone() });
        
        // Sync again - should be up to date
        let result = sync_service.sync_file(&file_path).await.expect("Failed to sync");
        assert_eq!(result, SyncResult::UpToDate { filepath: path_str.clone() });
        ...
        let result = sync_service.sync_file(&file_path).await.expect("Failed to sync");
        assert_eq!(result, SyncResult::Deleted { filepath: path_str.clone() });
        
        // Verify it's gone from database
        assert!(!db_service.document_exists(&path_str).expect("Failed to check"));
    }
    ...
    async fn test_sync_directory() {
        ...
        // Sync directory
        let results = sync_service.sync_directory(temp_dir.path()).await.expect("Failed to sync directory");
        
        // Should have 3 imports
        let imports = results.iter().filter(|r| matches!(r, SyncResult::Imported { .. })).count();
        assert_eq!(imports, 3);
        
        // Sync again - all should be up to date
        let results = sync_service.sync_directory(temp_dir.path()).await.expect("Failed to sync directory");
        let up_to_date = results.iter().filter(|r| matches!(r, SyncResult::UpToDate { .. })).count();
        assert_eq!(up_to_date, 3);
        
        // Check that we have results for all files
        for (file_path, _) in &files {
            let full_path = temp_dir.path().join(file_path).to_string_lossy().to_string();
